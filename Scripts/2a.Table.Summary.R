...
# replace NA by "-"
s1b[is.na(s1b)] = "-"
s1b$`Depth at buzzes (m)`[c(1:2,4:5)] = "-"
...
s2  = s1b %>%
  ...
  rename(n_daily_drops = 'n daily drops') 
...
s2$ID[s2$ID == "7617: 2015"] = "7617"
s2$ID[s2$ID == "7618: 2015"] = "7618"
s2$ID[s2$ID == "Mara: 2014"] = "Mara"


# import correction factor calculated for the 3 Acousondes
#---------------------------------------------------------
options(digits=10)
corr <- readRDS("./Rdata/corr_factor_nsat-nacou.rds")
...
dur_hours_new
dur_hours_new = dur_hours_new %>%
  mutate(ID = case_when(ID == "3965: 2013" ~ "3965",
                        ID == "Mara: 2014" ~ "Mara",
                        ID == "Thora" ~ "Thora",
                        ID == "7617: 2015" ~ "7617",
                        ID == "7618: 2015" ~ "7618",
                        ID == "Frida" ~ "Frida",
                        ID == "Eistla" ~ "Eistla")) %>%
  filter(ID != "3965") # already present in new dataset so rmeoved
...
s2
mean(as.numeric(s2$`Food consumption (%biomass/d)`),na.rm=T) # mean: 0.38%
