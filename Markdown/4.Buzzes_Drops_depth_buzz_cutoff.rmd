---
title: "Buzzes and drops with depth and first buzz cutoff"
...
# remove dives occurring before the first buzz
acou = acou[!acou$ptt=="3965",] %>%
  group_by(ptt) %>%
...
acou = acou[acou$keep.delete=="keep",]
table(acou$keep.delete)
acou$ptt = droplevels(acou$ptt)

# number of dives left per ID
table(acou$ptt)

# number of ST drops left per ID
table(acou$ptt[acou$type2=="start"])
acou = data.frame(acou)

# extract day at release, day of 1st buzz, day of 1st drop
time = acou %>%
  group_by(ptt) %>%
  summarise("day_first_buzz"=head(dateTime[buzz!=0],1),
            "day_first_drop"=head(dateTime[type2=="start" & !is.na(type2)],1))
row = data.frame("ptt"="3965",
                 "day_first_buzz"=NA,
...
## Duration between first buzz and first drop (h)
```{r, echo=F}
time$day_first_buzz = as.POSIXct(strptime(time$day_first_buzz,
                                     format="%Y-%m-%d %H:%M:%S"), tz="GMT")
time$hours_first_buzz = as.numeric(difftime(time$day_first_drop, 
                                            time$day_first_buzz, units="hours"))
time[,c(1,4)]
```
...
# remove events occurring before first buzz when acousonde
# eistla = setDT(event[event$ptt=="3962-Eistla",])[start %between% c(time$day_first_buzz[time$ptt=="Eistla"], 
#                                                                tail(event$end[event$ptt=="7925-Thora"],1))]
eistla = setDT(event[event$ptt=="3962-Eistla",])[start %between% c(time$day_first_buzz[time$ptt=="Eistla"], 
                                                               "2016-08-26 14:29:00")]
frida = setDT(event[event$ptt=="3965-Frida",])[start %between% c(time$day_first_buzz[time$ptt=="Frida"], 
                                                               "2015-09-10 17:21:00")]
frida = setDT(event[event$ptt=="3965-Frida",])[start %between% c(time$day_first_buzz[time$ptt=="Frida"], 
                                                               "2015-09-10 17:21:00")]
mara = setDT(event[event$ptt=="7618-Mara",])[start %between% c(time$day_first_buzz[time$ptt=="Mara"], 
                                                               "2014-08-24 19:26:00")]
thora = setDT(event[event$ptt=="7925-Thora",])[start %between% c(time$day_first_buzz[time$ptt=="Thora"], 
                                                               "2014-08-16 07:43:00")]
event = rbind(eistla,frida,mara,thora,event[event$ptt=="3965-TDR",],event[event$ptt=="7618-15",])
...
```{r, echo=FALSE, fig.pos="placeHere", fig.height=4, fig.cap="Histogram of the number of ST drops per individual after removing drops above 5 m and dives before first buzz."}
...
```{r, echo=FALSE, fig.pos="placeHere", fig.height=4, fig.cap="Histogram of the number of daily ST drops per individual after removing drops above 5 m and dives before first buzz."}
...
```{r, echo=FALSE, fig.pos="placeHere", fig.height=4, fig.cap="ST drop depth according to ST."}
ggplot(acou, aes(x=stp, y=-depth.x,colour=ptt)) +
  geom_point(aes(colour=ptt), size=0.3) + 
  labs(x="STP", y="Depth (m)") +
  scale_colour_tq() +
  facet_wrap(.~ptt) +
  theme_tq() +
  theme(legend.position="none")
